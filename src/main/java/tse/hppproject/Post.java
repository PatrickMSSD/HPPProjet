...
public class Post implements Comparable<Post> {
	...
	// utilisation de la table de correspondance post list de commentaire pour
	// compter le score
	...
	public Post(String str, long actual_time, Map<Long, ArrayList<Comment>> IDPost2Com) {
		...
		this.ts = Long.parseLong(string[0].substring(0, string[0].indexOf(".")).replace("T", "").replace("-", "")
				.replace(":", "").replace(" ", ""));
	...
	public void change_total_score() {
		...
		this.score_total = (int) (ts-actual_time<2400000 ? 0 : 10 -((ts-actual_time)%240000));
		...
		//System.out.println("time =" + (res - start));
	}
	...
	public String toString() {
		return "Post [ts=" + ts + ", post_id=" + post_id + ", user_id=" + user_id + ", user=" + user + ", score_total="
				+ score_total + "]";
	}
